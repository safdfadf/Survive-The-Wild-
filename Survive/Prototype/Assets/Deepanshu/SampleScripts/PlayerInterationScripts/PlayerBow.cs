...
public class PlayerBow : BaseWeapon
{
    ...
    protected override void Awake()
    {
        ...
        _bowCollider = GetComponent<Collider>();
        base.Awake();
    }
    ...
    public IEnumerator MoveObject(Transform obj, Vector3 start, Vector3 end, float duration)
    {
        float t = 0f;
        obj.position = start;
        ...
        while (t < duration)
        {
            ...
            // Smooth movement (optional: replace with normalized for linear)
            float eased = Mathf.SmoothStep(0f, 1f, normalized);

            obj.position = Vector3.Lerp(start, end, eased);
            yield return null;
        }
