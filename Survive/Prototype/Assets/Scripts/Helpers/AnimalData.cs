...
public class AnimalData
{
    ...
    public AnimalData(AnimalSo animalSo, Bounds regionBounds,ScheduleManager scheduleManager, AnimalHandler animalHandler)
    {
       AnimalHandler = animalHandler;
        ...
        if(currentZone == null){Debug.Log(currentZone);return;}
    ...
    public void OnHourChanged(int hour)
    {
        ...
        else
        {
            ...
            if (currentSchedule == null) {Debug.Log("current Schedule is null"); return;}
    ...
    {  
        ...
        CurrentPos = pos;
        // IF current pos lies in active zone spawn it at bounderies and then it will reach the zone 
        
        if (IsSpawned)// if already spawned which means it was in active 
        {
            AnimalBase animal = AnimalInstance.GetComponent<AnimalBase>();
            if(!animal.isMoving)
                animal.SetIsMoving(false);// ovveride move 
            isZoneTraveling = true;
            animal.MoveTo(CurrentPos.Value,onArrived: () => 
                TrackHandler.instance.CreateTracks(_lastSegment,Specie,currentState,AnimalSo));
            AnimalUI.transform.SetParent(animal.gameObject.transform);
            animal.ToglleEatAnim(true);// based on the zone chnage the anim
            return;
        }
        else if (ChunkRepo.instance.CheckPosInActiveChunk(CurrentPos.Value))
        {
            // spawn me and move to the current pos 
            // call global pool 
            ...
            animal.MoveTo(CurrentPos.Value);// improve further by adding a delay and make them reach at a proper time 
        }
    ...
    public void ChangeState(AnimalState state)
    {
        ...
        if(!IsSpawned)return;
        AnimalBase animal = AnimalInstance.GetComponent<AnimalBase>();
        animal.ActivateState(currentState);
    }
