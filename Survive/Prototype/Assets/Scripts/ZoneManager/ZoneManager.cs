...
// this script will be responsible for instantiate the zones manager will spawn the zones under the specific radius of the map 
public class ZoneManager : MonoBehaviour
{
    ...
    // instead of creating drink zone get the zone by
    private void StoreDrinkingZone()
    {
        _drinkingZone = FindObjectsByType<Zone>(FindObjectsSortMode.InstanceID).Where(z=>z.waterBodyType != WaterBody.Null).ToList();
    ...
    private void CreateZone(Activity type, Vector3 position) // this function will creteate zones
    {
        ...
        if (!_zoneByType.ContainsKey(type)) _zoneByType[type] = new List<Zone>();// checks if there is a list by that key if not it creates the list
